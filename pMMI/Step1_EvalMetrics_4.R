...
###Loading Data
stations=read.csv('~/Documents/R/ASCI/DATA/algae.site.data.04052019.csv',header=TRUE,strip.white=TRUE,check.names=FALSE) #this is most recent + the not most recent sites
row.names(stations)<-stations$SampleID_old
stations<-stations[order(row.names(stations)),]

traits=read.csv('~/Documents/R/ASCI/DATA/algae.traits.data.04052019.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, na.strings=c("","NA"))

if (diatoms == T) { 
taxonomy_pa=read.csv('~/Documents/R/ASCI/DATA/species_matrix/diatoms.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1)
...
taxonomy_count=read.csv('~/Documents/R/ASCI/DATA/species_matrix/diatoms.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1) 
...
if (sba == T) { 
taxonomy_pa=read.csv('~/Documents/R/ASCI/DATA/species_matrix/sba.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1)
taxonomy_pa<-taxonomy_pa[order(row.names(taxonomy_pa)),]
taxonomy_pa<-as.data.frame(ifelse(taxonomy_pa>0,1,0))
taxonomy_pa$SampleID_old<-row.names(taxonomy_pa)
taxonomy_count=read.csv('~/Documents/R/ASCI/DATA/species_matrix/sba.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1) 
taxonomy_count<-taxonomy_count[order(row.names(taxonomy_count)),]
taxonomy_count<-as.data.frame(ifelse(taxonomy_count>0,1,0))
taxonomy_count$SampleID_old<-row.names(taxonomy_count) }

if (hybrid == T) { 
taxonomy_pa=read.csv('~/Documents/R/ASCI/DATA/species_matrix/hybrid.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1)
taxonomy_pa<-taxonomy_pa[order(row.names(taxonomy_pa)),]
taxonomy_pa<-as.data.frame(ifelse(taxonomy_pa>0,1,0))
taxonomy_pa$SampleID_old<-row.names(taxonomy_pa)
taxonomy_count=read.csv('~/Documents/R/ASCI/DATA/species_matrix/hybrid.m.species.csv',header=TRUE,strip.white=TRUE,check.names=FALSE, row.names=1) 
taxonomy_count<-taxonomy_count[order(row.names(taxonomy_count)),]
taxonomy_count<-as.data.frame(ifelse(taxonomy_count>0,1,0))
taxonomy_count$SampleID_old<-row.names(taxonomy_count) }
...
# 25 total taxa 
# 12 are NAHON 
# 17 have nitrogen designation 
ddply(.data = other_combined3[1:25,], .variables = ~SampleID_old,.fun = summarize,
              OrgN.NAHON.richness=sum(na.omit(NitrogenUptakeMetabolism=='NAHON')) )
ddply(.data = other_combined3[1:25,], .variables = ~SampleID_old,.fun = summarize,
      prop.spp.OrgN.NAHON=sum(na.omit(NitrogenUptakeMetabolism=='NAHON'))/length(NitrogenUptakeMetabolism) )
      # 0.48 
ddply(.data = other_combined3[1:25,], .variables = ~SampleID_old,.fun = summarize,
      prop.spp.OrgN.NAHON=sum(na.omit(NitrogenUptakeMetabolism=='NAHON'))/length(na.omit(NitrogenUptakeMetabolism) ))
      # 0.71
ddply(.data = other_combined3[1:25,], .variables = ~SampleID_old,.fun = summarize,
      length(na.omit(NitrogenUptakeMetabolism)) )

### other Metric Calculations ------------------------------------------------------------------------------------------------------------

if (diatoms == T) {
metrics.d=ddply(.data = other_combined3.d, .variables = ~SampleID_old,.fun = summarize,
...
if (sba == T ) {
metrics.sba=ddply(.data = other_combined3.sba, .variables = ~SampleID_old,.fun = summarize,
...
if (hybrid == T) { 
metrics.h=ddply(.data = other_combined3, .variables = ~SampleID_old,.fun = summarize,
...
if ( diatoms == T ) { metrics <- metrics.d}
if ( sba == T ) { metrics <- metrics.sba}
if ( hybrid == T ) { metrics <- metrics.h}

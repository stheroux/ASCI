...
del<-which(foo$Freq.Max>40)
if (length(foo)>0) {foo<-foo[-del,]}
#del<-which(foo$Freq.Mode>40)
#if (length(foo)>0) {foo<-foo[-del,]}
...
out1<-subset(out1, out1$V2<0.23)
out1<-subset(out1, out1$V3<6)
...
win.metrics<-c(  "prop.spp.Green_raw",
                 "cnt.spp.IndicatorClass_Cu_high_raw",
                 "cnt.spp.IndicatorClass_DOC_high_raw",
                 "prop.spp.BCG45_raw",
...
win.metrics.raw<-c("prop.spp.Green_raw",
                   "cnt.spp.IndicatorClass_DOC_high_raw",
                   "prop.spp.BCG45_raw",
                   NULL) 
